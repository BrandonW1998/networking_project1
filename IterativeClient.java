...
public class IterativeClient {
	...
	public static void main(String[] args) {
		...
		Thread[] threadArr = new Thread[25];	//Array of threads, 25 client maximum
		...
		try {
			...
			while (command != 7) {
				...
				for(int i = 0; i < count; i++) {
					Thread thread = new Thread(new IterativeClientThreading(address, port, command));
					threadArr[i] = thread;
				}
...
class IterativeClientThreading implements Runnable {
	String address;
	int port;
	int command;
	String response;
	...
	public IterativeClientThreading(String address, int port, int command) {
